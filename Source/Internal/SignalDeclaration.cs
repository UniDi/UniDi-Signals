...
namespace UniDi
{
    public class SignalDeclaration : ITickable, IDisposable
    {
        ...
        readonly UniDiSettings.SignalSettings _settings;
        ...
        void FireInternal(List<SignalSubscription> subscriptions, object signal)
        {
                ...
                )
            {
                ...
                {
                    Log.Warn("Fired signal '{0}' but no subscriptions found!  If this is intentional then either add OptionalSubscriber() to the binding or change the default in UniDiSettings", signal.GetType());
                }
                else if (_missingHandlerResponses == SignalMissingHandlerResponses.Throw)
                {
                    throw Assert.CreateException(
                        "Fired signal '{0}' but no subscriptions found!  If this is intentional then either add OptionalSubscriber() to the binding or change the default in UniDiSettings", signal.GetType());
                }
            }
